...
public class EhcacheUtils {
	...
	public static List<Map<String,Object>> queryCatchContent(){
		List<Map<String,Object>> catchList=new ArrayList<Map<String,Object>>();
		Cache cah=cacheManager.getCache("cache");
		List<String> keyList=cah.getKeys();
		
		for(String name:keyList){
			Map<String,Object> catchMap=new HashMap<String,Object>();
			catchMap.put("catchType","数据库表结构缓存");
			catchMap.put("key",name);
			catchMap.put("value",cah.get(name));
			catchList.add(catchMap);
		}
		
		cah=cacheManager.getCache("passwordRetryCache");
		keyList=cah.getKeys();
		for(String name:keyList){
			Map<String,Object> catchMap=new HashMap<String,Object>();
			catchMap.put("catchType","重复登录缓存");
			catchMap.put("key",name);
			catchMap.put("value",cah.get(name));
			catchList.add(catchMap);
		}
		
		cah=cacheManager.getCache("shiro-activeSessionCache");
		keyList=cah.getKeys();
		for(String name:keyList){
			Map<String,Object> catchMap=new HashMap<String,Object>();
			catchMap.put("catchType","activeSessionCache");
			catchMap.put("key",name);
			catchMap.put("value",cah.get(name));
			catchList.add(catchMap);
		}
		
		cah=cacheManager.getCache("shiro-kickout-session");
		keyList=cah.getKeys();
		for(String name:keyList){
			Map<String,Object> catchMap=new HashMap<String,Object>();
			catchMap.put("catchType","kickoutSessionCache");
			catchMap.put("key",name);
			catchMap.put("value",cah.get(name));
			catchList.add(catchMap);
		}
