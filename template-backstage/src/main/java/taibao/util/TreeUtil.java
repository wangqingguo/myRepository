...
public class TreeUtil {
	...
	private List<TreeObject> getChildList(List<TreeObject> list, TreeObject t) {
		...
		while (it.hasNext()) {
			TreeObject n = (TreeObject) it.next();
			if ((int)n.getParentId() == (int)t.getId()) {
				tlist.add(n);
			}
		}
		return tlist;
	 ...
	 * @param args
	 */
	public List<TreeDept> getChildTreeDepts(List<TreeDept> list,String praentId) {
		List<TreeDept> returnList = new ArrayList<TreeDept>();
		for (Iterator<TreeDept> iterator = list.iterator(); iterator.hasNext();) {
			TreeDept t = (TreeDept) iterator.next();
			// 一、根据传入的某个父节点ID,遍历该父节点的所有子节点
	 ...
	 * @param TreeObject
	 */
	private void  recursionFn(List<TreeDept> list, TreeDept t) {
		List<TreeDept> childList = getChildList(list, t);// 得到子节点列表
		t.setChildren(childList);
		for (TreeDept tChild : childList) {
			if (hasChild(list, tChild)) {// 判断是否有子节点
				//returnList.add(TreeObject);
				Iterator<TreeDept> it = childList.iterator();
				while (it.hasNext()) {
					TreeDept n = (TreeDept) it.next();
					recursionFn(list, n);
				}
			}
		}
	...
	// 得到子节点列表
	private List<TreeDept> getChildList(List<TreeDept> list, TreeDept t) {
		
		List<TreeDept> tlist = new ArrayList<TreeDept>();
		Iterator<TreeDept> it = list.iterator();
		while (it.hasNext()) {
			TreeDept n = (TreeDept) it.next();
	...
	List<TreeDept> returnList1 = new ArrayList<TreeDept>();
	/**
     * 根据父节点的ID获取所有子节点
     * @param list 分类表
     * @param typeId 传入的父节点ID
     * @param prefix 子节点前缀
     */
    public List<TreeDept> getChildTreeDepts(List<TreeDept> list, String typeId,String prefix){
        if(list == null) return null;
        for (Iterator<TreeDept> iterator = list.iterator(); iterator.hasNext();) {
            TreeDept node = (TreeDept) iterator.next();
            // 一、根据传入的某个父节点ID,遍历该父节点的所有子节点
    ...
    private void recursionFn(List<TreeDept> list, TreeDept node,String p) {
        List<TreeDept> childList = getChildList(list, node);// 得到子节点列表
        if (hasChild(list, node)) {// 判断是否有子节点
            returnList1.add(node);
            Iterator<TreeDept> it = childList.iterator();
            while (it.hasNext()) {
                TreeDept n = (TreeDept) it.next();
                n.setDeptName(p+n.getDeptName());
                recursionFn(list, n,p+p);
            }
        } else {
            returnList1.add(node);
	...
	// 判断是否有子节点
	private boolean hasChild(List<TreeDept> list, TreeDept t) {
		return getChildList(list, t).size() > 0 ? true : false;
	}
